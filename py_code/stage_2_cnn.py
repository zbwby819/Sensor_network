...
def cnn_model_simple(): # input/output = num of sensors 
    input_shape = (84, 84*4, 3)        
    #sensor_matrix3 = Input(shape=(num_sensors, num_sensors))
    r_input = Input(shape=(input_shape[0], input_shape[1], input_shape[2]))
    robot_cnn = Conv2D(256, activation='selu')(r_input)
    robot_cnn = Conv2D(512, activation='selu')(robot_cnn)
...
def cnn_model(): # input/output = num of sensors 
    ...
    a_output = Dense(256, activation='selu', name='actor_mlp_1')(Flatten()(robot_cnn))
    a_output = Dense(64, activation='selu', name='actor_mlp_2')(a_output)
    a_output = Dense(action_dim, activation='tanh', name='actor_output')(a_output)
...
def robot_khop_model(): # input/output = num of sensors 
    ...
    a_output = Dense(64, activation='relu', name='policy_mlp')(Flatten()(gnn_output[-1]))
    a_output1 = Dense(action_dim, activation='linear', name='Out_mean')(a_output)
    
    c_output = Dense(64, activation='relu', name='value_mlp')(Flatten()(gnn_output[-1]))
    c_output1 = Dense(1, activation='linear', name='Out_value')(c_output)
...
def cal_admatrix(pos, env_index, num_sensors=10):
    robot_loc = pos
    sensor_loc = np.load('stage2_envs/env_{}_9_1_loc.npy'.format(env_index+1))
    ad_matrix1 = np.zeros((10,10))
    ad_matrix2 = np.zeros((10,10))
    ad_matrix3 = np.zeros((10,10))
    ad_matrix4 = np.zeros((10,10))
    ad_matrix1[:9,:9] = np.load('stage2_envs/env_{}_9_1_ad1.npy'.format(env_index+1))
    ad_matrix2[:9,:9] = np.load('stage2_envs/env_{}_9_1_ad2.npy'.format(env_index+1))
    ad_matrix3[:9,:9] = np.load('stage2_envs/env_{}_9_1_ad3.npy'.format(env_index+1))
    ad_matrix4[:9,:9] = np.load('stage2_envs/env_{}_9_1_ad4.npy'.format(env_index+1))
...
class PPO:
    ...
    def act(self, state, test=False):
        ...
        output = tf.clip_by_value(output, -1, 1)
        dist = self.get_dist(output.astype(np.float32))
    ...
    def test3(self):
        ...
        while not done and steps < 100:
            ...
            p_new = np.zeros((2,1))
            z1 = np.true_divide(robot2target_angle[0], np.sqrt(robot2target_angle[0]**2 + robot2target_angle[1]**2) + ZERO_TOLERANCE) + np.random.random()*0.2
            z2 = np.true_divide(robot2target_angle[1], np.sqrt(robot2target_angle[0]**2 + robot2target_angle[1]**2) + ZERO_TOLERANCE) + np.random.random()*0.2
            able_see = in_view(self.dis_env, robot_loc, target_loc)
            if able_see:
                able_move = True
